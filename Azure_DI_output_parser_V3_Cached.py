...
def load_master_prompt_v3():
    ...
    try:
        # Use the Databricks volume path for the prompt file
...
class SimpleDocumentProcessorV3:
    def __init__(self, filepath, cached_model=None):
        ...
        self.cached_model = cached_model  # NEW: Store cached model
        self._load_data()
    ...
    def analyze_document_with_llm(self, markdown_content, segment_id):
        ...
        try:
            if self.cached_model:
                ...
                # Pass only the document content since system instruction is in cache
                gemini = llm.select_model(llm_registry.vertex.Gemini.PRO_2_5)
                
                # The cache contains the system instruction (prompt)
                # We only send the document content
                message = gemini.invoke(
                    f"Document Content to Analyze:\n{markdown_content}",
                    # Pass the cache object directly to itsai
...
def create_cached_model(system_instruction, ttl="1800s"):
    ...
    Args:
        ...
        ttl: Time to live for the cache (default: 30 minutes)
    ...
    try:
        ...
        # Return the cache object itself, not a model with cache
        # The cache will be used in the invoke call
        return cache
...
if __name__ == '__main__':
    ...
    print("🚀 DOCUMENT PROCESSOR V3 WITH CACHING")
    print("="*60)
    ...
    print("- Bounding box location tracking")
    ...
    # Output files will be saved in the same PI folder
    ...
    for json_file in sorted(json_files):
        ...
        # Create output filename with _cached suffix
        import os
        base_name = os.path.basename(json_file).replace('.pdf.json', '')
        output_filename = f"{output_path}{base_name}_cached_analysis.txt"
