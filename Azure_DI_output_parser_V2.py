...
def analyze_document_with_llm(document_content: str, segment_id: str):
    """
    Analyze a document segment using the Master Prompt V2 and LLM.
    
    Args:
        document_content (str): The concatenated content of all pages in the segment
        segment_id (str): The identifier for this document segment
    
    Returns:
        str: LLM analysis result in JSON format
    """
    print(f"\n🤖 Running LLM analysis for {segment_id}...")
    
    # Combine the master prompt with the document content
...
def process_document_segment(segment_content: str, segment_id: str, page_range: str):
    ...
    # Run LLM analysis using Master Prompt V1
    llm_result = analyze_document_with_llm(segment_content, segment_id)
    print(f">>> LLM Result for entire document:")
    print(f"{'='*80}")
    print(llm_result)
    print(f"{'='*80}")
    ...
    def _process_document_segments(self, output_filepath=None):
        ...
        try:
            ...
            for segment_id, segment_info in self.document_segments.items():
                ...
                # Run LLM analysis and save to output file
                if output_file:
...
if __name__ == '__main__':
    ...
    # Process all Personal_Information JSON files with LLM Analysis (V2)
    json_files = glob.glob("Personal_Information_*.pdf.json")
    
    print("🚀 PROCESSING ALL FILES WITH LLM ANALYSIS (V2)")
    print("="*80)
    print("V2 Features:")
    print("- Document segmentation with improved boundary detection")
    print("- LLM analysis using Master Prompt V2")
    print("- Sensitive information detection")
    print("- CV/Resume identification")
    print("- Offensive language detection")
    print("- Name extraction with context")
    print("- World Bank Access to Information Policy compliance")
    print("="*80)
    
    for json_file in sorted(json_files):
        print(f"\n🔍 Processing: {json_file}")
        
        # Create output filename with V2 naming convention
        base_name = json_file.replace('.pdf.json', '')
        output_filename = f"{base_name}_V2_analysis.txt"
    ...
    print(f"\n🎯 All files processed with LLM Analysis (V2)!")
    print(f"   Output files named: Personal_Information_#_V2_analysis.txt")
